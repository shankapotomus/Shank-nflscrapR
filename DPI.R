...
PIdf <- PI0%>%
  ...
  mutate(WRlength = str_length(WR))%>%
  select(description,posteam,QB,WR,penalty_player_name,DPI, OPI,penalty_yards,down,ydstogo,yardline_100,ep,epa,wp,wpa)%>%
...
ggplot(QBDPIdf, aes(dropbacks,DPIepa,color = team,fill=team)) + 
